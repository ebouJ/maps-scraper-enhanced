...
type Config struct {
	...
	ExtraReviews             bool
	// Go-Lead-API Integration Settings (simplified)
	LeadAPIEnabled  bool
	LeadAPIURL      string
	LeadAPIKey      string
	LeadAPITimeout  time.Duration
}
...
func ParseConfig() *Config {
	...
	// Go-Lead-API Integration flags (simplified)
	flag.BoolVar(&cfg.LeadAPIEnabled, "lead-api-enabled", false, "enable go-lead-api integration for lead qualification")
	flag.StringVar(&cfg.LeadAPIURL, "lead-api-url", "", "go-lead-api base URL (e.g., 'https://lead-api.example.com')")
	flag.StringVar(&cfg.LeadAPIKey, "lead-api-key", "", "go-lead-api API key for authentication")
	flag.DurationVar(&cfg.LeadAPITimeout, "lead-api-timeout", 30*time.Second, "go-lead-api request timeout")
	...
	// Go-Lead-API environment variables
	if cfg.LeadAPIURL == "" {
		cfg.LeadAPIURL = os.Getenv("LEAD_API_URL")
	}
	
	if cfg.LeadAPIKey == "" {
		cfg.LeadAPIKey = os.Getenv("LEAD_API_KEY")
	}
	
	// Parse boolean environment variables
	if !cfg.LeadAPIEnabled && os.Getenv("LEAD_API_ENABLED") == "true" {
		cfg.LeadAPIEnabled = true
	}
	...
	// Go-Lead-API validation
	if cfg.LeadAPIEnabled {
		if cfg.LeadAPIURL == "" {
			panic("Lead API URL must be provided when lead API is enabled")
		}
		if cfg.LeadAPIKey == "" {
			panic("Lead API key must be provided when lead API is enabled")
		}
		if cfg.LeadAPITimeout < time.Second {
			panic("Lead API timeout must be >= 1 second")
		}
	}
